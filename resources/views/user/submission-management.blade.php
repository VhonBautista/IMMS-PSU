        ...
        <div class="p-6">
            ...
            <form class="space-y-4 md:space-y-6" method="POST" action="{{ route('submission.store') }}" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <select name="course_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 max-h-10 overflow-y-auto" required>
                        ...
                        {{-- @foreach($courses as $course)
                            <option value="{{ $course->id }}" @if($course->course_id == $course->id) selected @endif>{{  $course->campus->campus_name . ' - ' . $course->course_name }}</option>
                        @endforeach --}}
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="department_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 max-h-10 overflow-y-auto" required>
                        ...
                        {{-- @foreach($departments as $department)
                            <option value="{{ $department->id }}" @if($department->department_id == $department->id) selected @endif>{{  $department->campus->campus_name . ' - ' . $department->department_name }}</option>
                        @endforeach --}}
                    </select>
                </div>
                ...
                <div class="w-full">
                    ...
                    <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" aria-describedby="file_input_help" name="pdf_path" type="file" accept=".pdf, .docx, .png, .jpg, .jpeg" required>
        ...
        <div class="w-full md:w-3/4">
            <div class="bg-white p-6 rounded-lg">
                ...
                <div class="flex w-full px-0 md:px-4 flex-wrap items-center justify-between">
                    <form action="{{ route('submission_management') }}" method="GET" id="search-form-submission" class="w-full mb-3 md:w-auto md:mb-0 pe-1 sm:me-3">
                    ...
                    <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'submit-modal')" class="inline-flex sm:hidden mt-3 w-full lg:w-auto px-3 py-2.5 text-sm font-medium items-center bg-gray-800 rounded-lg hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 dark:bg-gray-600 dark:hover:bg-gray-800 dark:focus:ring-gray-800 justify-center text-white">
                                        ...
                                        @forelse($pendingMaterials as $pendingMaterial)
                                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                    ...
                                                    <div class="font-medium text-xs text-gray-500 capitalize truncate ... max-w-[200px]">
        ...
        <div class="w-full md:w-1/4 mb-3">
            <div class="bg-white p-6 rounded-lg">
                <div class="flex flex-col justify-center items-center">
                    <div class="text-cente text-center">
                        <p class="text-sm lg:text-lg text-gray-700 font-semibold mb-1 capitalize">{{ __('Total material you have submitted') }}</p>
                        <p class="text-7xl text-gray-800 font-bold">{{ $totalSubmittedByUser }}</p>
                    </div>
                    
                    <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'submit-modal')" class="hidden sm:inline-flex my-3 w-full px-3 py-2.5 text-sm font-medium items-center bg-gray-800 rounded-lg hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 dark:bg-gray-600 dark:hover:bg-gray-800 dark:focus:ring-gray-800 justify-center text-white">
                        <svg class="w-4 h-4 me-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M.188 5H5V.13a2.96 2.96 0 0 0-1.293.749L.879 3.707c-.358.362-.617.81-.753 1.3C.148 5.011.166 5 .188 5ZM14 8a6 6 0 1 0 0 12 6 6 0 0 0 0-12Zm2 7h-1v1a1 1 0 0 1-2 0v-1h-1a1 1 0 0 1 0-2h1v-1a1 1 0 0 1 2 0v1h1a1 1 0 0 1 0 2Z"/>
                            <path d="M6 14a7.969 7.969 0 0 1 10-7.737V2a1.97 1.97 0 0 0-1.933-2H7v5a2 2 0 0 1-2 2H.188A.909.909 0 0 1 0 6.962V18a1.969 1.969 0 0 0 1.933 2h6.793A7.976 7.976 0 0 1 6 14Z"/>
                        </svg>
                    ...
                    <div class="w-full border-t pt-2">
                        <canvas id="statusPieChart" width="100%"></canvas>
                    </div>
                </div>
            </div>
        </div>
