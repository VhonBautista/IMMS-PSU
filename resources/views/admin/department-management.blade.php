<x-app-layout>
    <x-slot name="header">
            ...
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        ...
                        <span class="ml-1 font-semibold text-sm lg:text-lg text-gray-700 dark:text-gray-400">{{ __('Department Management') }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
            
            <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'add-department-modal')" class="px-3 py-2 text-sm font-medium text-center inline-flex items-center text-white bg-gray-800 rounded-lg hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 dark:bg-gray-600 dark:hover:bg-gray-800 dark:focus:ring-gray-800">
     ...
     <x-modal name="add-department-modal" focusable>
        <div class="p-6">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                {{ __('Add New Department') }}
            </h3>

            <form class="space-y-4 md:space-y-6 w-full" method="POST" action="{{ route('admin.department_management.store') }}">
                @csrf
                
                <div class="w-full">
                    <label class="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">{{ __('Department Name') }}</label>
                    <input type="text" name="department_name" id="department_name" class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{ __('Enter the name for the department') }}" required>
    ...
    <div class="bg-white p-6 rounded-lg">
        <form action="{{ route('admin.department_management') }}" method="GET" id="search-form" class="flex flex-col justify-center md:flex-row md:justify-between">
                        ...
                        @forelse($departments as $department)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <div class="font-medium text-sm text-gray-800 dark:text-gray-200 capitalize">{{ $department->department_name }}</div>
                                </th>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('admin.department_management.edit', $department->id) }}" class="px-3 py-2 text-sm font-medium text-center inline-flex items-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                        ...
                                        <button type="button" onclick="setDeleteFormAction('department-id-input', {{ $department->id }})" x-data="" x-on:click.prevent="$dispatch('open-modal', 'delete-department-modal')" class="px-3 py-2 text-sm font-medium text-center inline-flex items-center text-white bg-red-600 rounded-lg hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-500 dark:bg-red-600 dark:hover:bg-red-800 dark:focus:ring-red-800">
    ...
    <x-modal name="delete-department-modal" :maxWidth="'md'" focusable>
        <div class="p-6">
            <form class="space-y-4 md:space-y-6" id="delete-department-form" method="POST" action="{{ route('admin.department_management.destroy') }}">
                @csrf
                @method('DELETE')
                ...
                <div class="text-center">
                    ...
                    <h3 class="mb-5 text-md font-medium text-gray-500 dark:text-gray-400">{{ __('Confirm deletion of this department? This action is irreversible.') }}</h3>
                </div>
