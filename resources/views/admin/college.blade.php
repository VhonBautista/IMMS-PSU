<x-app-layout>
            ...
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        ...
                        <span class="ml-1 font-semibold text-sm lg:text-lg text-gray-700 dark:text-gray-400">{{ __('Colleges') }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
    ...
    <x-modal name="add-campus-modal" focusable>
        <div class="p-6">
            ...
            <form class="space-y-4 md:space-y-6" method="POST" action="{{ route('college.store') }}">
                @csrf
                        ...
                        <div class="w-full lg:w-full">
                            ...
                            <div class="flex items-start w-full">
                                <select name="campus_id" class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 max-h-10 overflow-y-auto" required>
                                    ...
                                    <option value="{{ $campus->id }}">{{ $campus->id . ' - ' . $campus->campus_name }}</option>
...
{{-- Alert End --}}
    ...
    <div class="bg-white p-6 rounded-lg ">
        <form action="{{ route('admin.college') }}" method="GET" id="search-form" class="flex flex-col justify-center md:flex-row md:justify-between">
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('college.edit', $college->id) }}" class="px-3 py-2 text-sm font-medium text-center inline-flex items-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                        ...
                                        <button type="button" onclick="setDeleteCollegeFormAction({{ $college->id }})" x-data="" x-on:click.prevent="$dispatch('open-modal', 'delete-college-modal')" class="px-3 py-2 text-sm font-medium text-center inline-flex items-center text-white bg-red-600 rounded-lg hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-500 dark:bg-red-600 dark:hover:bg-red-800 dark:focus:ring-red-800">
     ...
     <x-modal name="delete-college-modal" :maxWidth="'md'" focusable>
        <div class="p-6">
            <form class="space-y-4 md:space-y-6" id="delete-college-form" method="POST" action="{{ route('college.destroy',$college->id) }}">
                @csrf
                @method('DELETE')
