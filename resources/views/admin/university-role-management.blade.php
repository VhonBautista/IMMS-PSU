<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'add-university-modal')" class="px-3 py-2 text-sm font-medium text-center inline-flex items-center text-white bg-gray-800 rounded-lg hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 dark:bg-gray-600 dark:hover:bg-gray-800 dark:focus:ring-gray-800">
    ...
    {{-- Add University Role Modal --}}
    <x-modal name="add-university-modal" focusable>
        <div class="p-6">
        ...
        <div class="max-w-7xl mt-3 mx-auto space-y-6">
            <div class="relative overflow-x-auto border sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @forelse($university_roles as $university)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <div class="font-medium text-sm text-gray-800 dark:text-gray-200 capitalize">{{ $university->university_role }}</div>
                                </th>
                                <td class="px-6 py-4 text-xs">
                                    {{ $university->description }}
                                </td>
                                <td class="px-6 py-4 text-xs capitalize">
                                    {{ $university->created_at->format('M d, Y h:i A') }}
                                </td>
                                <td class="px-6 py-4 text-xs capitalize">
                                    {{ $university->updated_at->format('M d, Y h:i A') }}
                                </td>   
    ...
    <x-modal name="delete-university-modal" :maxWidth="'md'" focusable>
        <div class="p-6">
            <form class="space-y-4 md:space-y-6" id="delete-university-form" method="POST" action="{{ route('admin.university_role_management.destroy') }}">
                ...
                <input type="hidden" name="id" id="university-id-input" value="">
