<x-app-layout>
    ...
    <x-modal name="add-evaluator-modal" focusable>
        <div class="p-6">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Add Matrix Evaluators</h3>
    ...
    <div class="p-8 bg-white rounded-lg mt-6">
        ...
        <div class="flex flex-wrap gap-5 pt-6">
            <div class="border border-1 p-3 md:p-6 bg-white rounded-lg flex-1">
                <h1 class="text-lg pb-3 border-b flex w-full items-center justify-between text-center md:text-start font-semibold leading-tight tracking-tight text-gray-900 dark:text-white">
                    {{ $matrix->matrix_name }}
                            ...
                            <ul class="ms-3 text-gray-500 dark:text-gray-400">
                                @forelse( $subMatrix->matrixItems->sortBy('item') as $matrixItem )
                                    <li class="flex justify-between items-center w-full pt-1">
                                        ...
                                        <a data-tooltip-target="tooltip-remove-item-{{ $matrix->id . '-' . $subMatrix->id . '-' . $matrixItem->id }}" href="{{ route('admin.matrix_management.item.remove', ['matrixItemId' => $matrixItem->id, 'matrixId' => $matrix->id]) }}" class="text-sm cursor-pointer text-red-600 hover:text-red-500">Remove</a>
                                        ...
                                        <div id="tooltip-remove-item-{{ $matrix->id . '-' . $subMatrix->id . '-' . $matrixItem->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                            Remove {{ $matrixItem->item }}
                                        </div>
                                        
                                        <div id="tooltip-text-{{ $matrix->id . '-' . $subMatrix->id . '-' . $matrixItem->id }}" role="tooltip" class="absolute z-10 max-w-[300px] invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                            {{ $matrixItem->text }}
                                        </div>
                                    </li>
                                @empty
                ...
                <div class="px-2 pt-3 text-gray-800 dark:text-gray-400">
                    @forelse( $matrix->evaluatorMatrices as $evaluatorMatrix )
                        <div class="flex w-full gap-6 justify-between items-center text-sm mb-4">
                            <div class="flex flex-col">
                                ...
                                <span class="text-sm font-normal text-gray-600 dark:text-gray-500">
                                    {{ $evaluatorMatrix->evaluator->description }}
                                </span>
                            </div>
                            
                            <a data-tooltip-target="tooltip-remove-{{ $evaluatorMatrix->evaluator->id . '-' . $evaluatorMatrix->matrix->id }}" href="{{ route('admin.matrix_management.remove', ['universityRoleId' => $evaluatorMatrix->evaluator->id, 'matrixId' => $evaluatorMatrix->matrix->id]) }}" class="cursor-pointer text-red-600 hover:text-red-500">Remove</a>
                        </div>
                        ...
                        <div id="tooltip-remove-{{ $evaluatorMatrix->evaluator->id . '-' . $evaluatorMatrix->matrix->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                            Remove {{ $evaluatorMatrix->evaluator->university_role }}
                        </div>
                    @empty
                        <div class="px-6 py-4 flex flex-col text-sm font-medium items-center text-center w-full text-gray-500 dark:text-gray-400">
