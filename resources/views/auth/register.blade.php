<x-guest-layout>
    <div class="p-6 space-y-4 md:space-y-6 sm:p-4">
        ...
        <form class="space-y-4 md:space-y-6" method="POST" action="{{ route('register') }}">
            ...
            <p class="text-sm text-center text-gray-500 dark:text-gray-300" id="file_input_help" style="margin-top: 12px !important;">{{ __('Caution: Accurate university details are crucial for verification by the admin. Failure to provide correct information may result in account suspension.') }}</p>
            ...
            <div class="flex items-start ms-1">
                <select name="university_role" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 max-h-10 overflow-y-auto" required>
                    ...
                    @foreach($universityRoles as $role)
                        <option value="{{ $role->id }}">{{ $role->university_role }}</option>
                    @endforeach
                </select>
            </div>
